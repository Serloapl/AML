...
def priorSample(modelNumber, samples):
    ...
    cov = sigma*0.5*np.ones((modelNumber,modelNumber)) + 0.5*sigma*np.eye(modelNumber)
    mean = np.zeros(modelNumber)
    #mean = 5*np.ones(modelNumber)
...
def computeEvidence(dataset, modelNumber, samples):
    ...
    for i in range(len(samples)):
        p = p + float(model(modelNumber, dataset, samples[i]))
...
for i,mx in enumerate(max):
    print "most probable set in M",i," ---> "
    print l[mx]
    print mx
for i,mn in enumerate(min):
    print "least probable set in M",i," ---> "
    print l[mn]
    print mn
